...
- The canonical plan file: `PLAN.md` for pipeline runs, or the file the user pointed at (copied into the repo as `RF-PLAN.md` if it lives outside). One file for the whole meeting; every revision lands in it. Call it `PLAN_FILE` below. For a meeting with purpose ELECTION, PLAN_FILE is `ELECTION.md` (template: `templates/ELECTION.template.md`) and the cap is 5.
...
- `MAX_PASSES` = 10 by default per meeting kind (L1-L3 meeting, L6 review meeting counted separately; 5 for the ELECTION meeting); whatever the Owner set is a hard cap, not a suggestion.
...
- **Election meetings:** the close binds every participating seat's `VERDICT: SAME PAGE` to one ELECTION.md hash and the frozen roster with zero open items; the log's closing record names, per seat, the source-record locator of its verdict (the seat's original response: captured output file and line, or native provider/runtime session and message/turn identifier), the ELECTION.md hash, the closed plan and contract hashes it binds, and the round; planning SAME PAGE advances to ELECT, and execution requires this closure and all existing Owner gates.
...
After the planning meeting closes and before any dispatch, in every gauntlet round, the seats hold a second council meeting with purpose ELECTION: this procedure run on `ELECTION.md` (template: `templates/ELECTION.template.md`) with a cap of 5 passes and the same revision, roster, item, log and recovery machinery. Each seat reads the evidence independently first -- the capability chart's cells, the SETUPS selector row AND the lane's selector-state row quoted verbatim, PRIORS, live quota on every pool with source, time, age and reset -- then the seats settle, per task: arm@effort, setup, agent count and assignments, ownership and dependencies, any S0 justification, cadence-probe eligibility and disposition, fallback conditions. The meeting closes on every participating seat's `VERDICT: SAME PAGE` bound to one ELECTION.md sha256 and the frozen roster with zero open items; the verdicts live in the append-only log, never inside the frozen file, and the log's closing record carries a source-record locator for each seat's verdict.

Planning SAME PAGE advances to ELECT; execution requires election closure and all existing Owner gates. Before each execution, including S0, the coordinator re-reads the election artifact and verifies its hash, unit, round, frozen roster, closed plan and contract hashes, task coverage, and dispatch configuration against the unanimous closing record -- taking the current unit, round, roster, task and proposed dispatch as independently established inputs, rehashing the actual plan and contracts, and checking every seat's source-backed closing verdict, identical election revision and zero open items; a coordinator's closing summary alone is insufficient. Missing, stale or mismatched evidence blocks execution. Recording a probe exemption grants no authority: cite the applicable policy or a scoped Owner override, and preserve outstanding debt. The coordinator reads the completed verification result before issuing execution; a failed, unavailable or malformed verification blocks the executor call. This is procedural enforcement; the mechanical gate is unchanged. Frozen decision artifacts (PLAN_FILE, the contracts, ELECTION.md) are protected from executor writes, and the existing quota freshness and admission rules apply before dispatch.

Re-entry: every round after the first reaches the election meeting (an instruction patch with unchanged executors is re-confirmed there; arm swaps and setup changes are elections); a pre-start re-route returns from dispatch to this meeting within the same round with its consumed passes preserved; a started attempt completes L5-L7 first; a contract change requires renewed planning closure, and a scope change keeps the Owner's re-plan gate.
